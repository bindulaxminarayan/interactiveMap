...
def _fetch_questions_from_db(category: str, subcategory: str, num_questions: int) -> List[Dict]:
    """Fetch questions from database by category and subcategory."""
    query = """
    ...
    try:
        ...
        logging.debug(
            "Querying for category=%s, subcategory=%s. Found %d questions",
            category, subcategory, len(question_rows)
        )
...
def get_quiz_questions(quiz_type: str, num_questions: int = 10) -> List[Dict[str, Any]]:
    """
    ...
    """
    logging.debug("Getting questions for quiz type: %s", quiz_type)
